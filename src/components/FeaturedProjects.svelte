...
>
  ...
  <div class="projects-container relative z-[50]">
    <div class="mb-16 flex flex-col justify-between gap-6 md:flex-row md:items-end">
      <div>
        <h2 class="font-display text-4xl text-[var(--color-on-surface)] sm:text-5xl">
          {title}
        </h2>
        <span class="mt-3 block text-sm font-medium tracking-[0.2em] text-[var(--color-on-surface-muted)] uppercase">
          {subtitle}
        </span>
      </div>
    ...
    <div class="grid grid-cols-1 gap-12">
      {#each projects as project, i (project.slug)}
        {@const isMatch = $activeFocus
          ? project.stack.some((s) => s.toLowerCase().includes($activeFocus!.toLowerCase())) ||
        ...
        >
          ...
          >
            ...
            {/if}
            <div class="absolute inset-0 z-10 bg-black/5 opacity-0 transition-opacity duration-300 group-hover/image:opacity-100"></div>
          </a>
          ...
          <div class="flex w-full flex-col justify-center py-4 md:w-2/5">
            ...
            <h3 class="font-display mb-4 text-3xl font-semibold text-[var(--color-on-surface)]">
              {project.title}
            </h3>
            <p class="text-base leading-relaxed text-[var(--color-on-surface-muted)]">
              {project.summary}
            </p>

            <div class="mt-8 flex flex-wrap gap-2">
              {#each project.stack as tool (`${project.title}-${tool}`)}
                <li class="list-none rounded bg-[var(--color-surface-container)] px-2.5 py-1.5 text-xs font-medium tracking-wide text-[var(--color-on-surface-muted)]">
                  {tool}
                </li>
              {/each}
            </div>
